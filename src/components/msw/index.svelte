...
<div class="msw-container">
  ...
  {#if show}
    ...
    <div
      ...
      <h2 class="msw-title">
        MSW-TOOLS控制台
        <span on:click|stopPropagation="{closeModal}" class="msw-close">X</span>
      </h2>
      <div class="msw-tabs">
        <div class="msw-tabs-head">
          <div class="msw-tabs-inner">
            {#each tabs as { name, code }, index (code)}
              <div
                class="msw-tabs-item {code === currentTab ? 'active' : ''}"
                on:click="{tabChange.bind(null, code)}">
                {name}
              </div>
            {/each}
          </div>
          <div class="msw-tabs-handle">
            <a on:click="{resetHandlers}" href="{null}" class="msw-reset"
              >配 置</a>
          </div>
        </div>
            ...
            <div class="msw-tabs-wrap">
              <div class="msw-handle-wrap">
                ...
                <div class="msw-handle-li">
                  ...
                  <input
                    bind:value="{reqTimes}"
                    on:focusout="{inputChange.bind(null, 'time')}"
                    type="text"
                    class="msw-handle-input"
                    placeholder="默认 1000 " />
                </div>
                <div class="msw-handle-li">
                  <div class="msw-handle-label">请求失败比例(%)</div>
                  <input
                    bind:value="{failRatio}"
                    on:focusout="{inputChange.bind(null, 'fail')}"
                    type="text"
                    class="msw-handle-input"
                    placeholder="默认 0 " />
                </div>
                <div class="msw-handle-li">
                  <div class="msw-handle-label">
                    当前状态码
                    <span
                      class="status-code {+statusCode === 200 ? '' : 'error'}">
                      [ {statusCode} ]
                    </span>
                  </div>
                </div>
                <div class="msw-handle-li">
                  ...
                  <a
                    on:click="{fileTrigger}"
                    href="{null}"
                    class="msw-handle-export">导 入</a>
                  <input
                    bind:this="{fileObj}"
                ...
                <div class="msw-handle-li">
                  ...
                  <a
                    on:click="{exportHandle}"
                    href="{null}"
                    class="msw-handle-export">导 出</a>
                </div>
                {#if !isProd}
                  <div class="msw-handle-li">
                ...
                {#if showMsg}
                  <div
                    class="msw-config-tips {msgType === 'error'
                      ? 'error'
                      : 'success'}">
                    {msgText}
                  </div>
